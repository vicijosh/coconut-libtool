...
@st.cache_data(ttl=3600)
def scattervis(bursts, freq_data, top_n, dark_mode=False):
    ...
    fig.update_layout(
        ...
        plot_bgcolor='#15171c' if dark_mode else 'white',
        paper_bgcolor='#15171c' if dark_mode else 'white',
        font=dict(color='#f5f5f7' if dark_mode else '#1d1d1f'),
        showlegend=False,
...
@st.cache_data(ttl=3600)
@st.cache_data(ttl=3600)
def linegraph(bursts, freq_data, top_n, running_total="", dark_mode=False):
    ...
    for i, column in enumerate(freq_data.columns[:top_n]):
        ...
        for _, row_data in bursts[bursts['label'] == column].iterrows():
            ...
            fig.add_annotation(
                ...
                showarrow=False,
                font=dict(color="#f5f5f7" if dark_mode else "#1d1d1f", size=12),
                align=align_value,
                valign=valign_value,
                textangle=270,
                row=row, col=col)
...
def burst_summary_frame(bursts, freq_data):
    summary = bursts.copy()
    years = list(freq_data.index)

    def year_from_position(value):
        try:
            position = int(value)
        except Exception:
            return value
        if 0 <= position < len(years):
            return int(years[position])
        return position

    if {"begin", "end"}.issubset(summary.columns):
        summary["begin_year"] = summary["begin"].apply(year_from_position)
        summary["end_year"] = summary["end"].apply(year_from_position)
    return summary
        ...
        with tab1:        
            if bursts.empty:
                ...
                if viz_selected == "Line graph": 
                    linegraph(bursts, freq_data, top_n, dark_mode=ui.is_dark_mode())
                    
                elif viz_selected =="Heatmap":
                    scattervis(bursts, freq_data, top_n, dark_mode=ui.is_dark_mode())
                ...
                burst_ai_frame = burst_summary_frame(bursts, freq_data)
                top_word_totals = (
                    freq_data.sum()
                ...
                payload = ai.table_payload(
                    ...
                    metadata={
                        ...
                        "top_words_requested": int(top_n),
                        "visualization": viz_selected,
                        "calculation_method": running_total,
                        "count_method": count_method,
                        "top_word_totals": top_word_totals,
                    },
                )
                ai.render_ai_insights(
                    "Burst Detection",
                    payload,
                    f"burst_detection_{col_name}_{top_n}",
                    "Explain which terms surged, when the bursts happened, and how to interpret the caveats.",
                )
