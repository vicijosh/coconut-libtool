...
runtime.ensure_nltk_data('punkt', 'wordnet', 'stopwords')

ui.apply_app_style()

ui.render_tool_menu()
ui.render_page_header("Bidirected Network", "Build a two-way keyword relationship network from your dataset.")
    ...
    try:
        ...
        with tab1:
            ...
            if freq_item.empty:
                ...
                restab = st.data_editor(restab, width="stretch", hide_index=True)
                res = restab[restab['Show'] == True] 

                payload = ai.table_payload(
                    "Bidirected Network",
                    "Association rules",
                    restab.drop(columns=["Show"], errors="ignore"),
                    metadata={
                        "keyword_column": keyword,
                        "normalization_method": method,
                        "display_method": dispmethod,
                        "support_threshold": float(supp),
                        "confidence_threshold": float(conf),
                        "max_itemset_length": int(maxlen),
                    },
                )
                ai.render_ai_insights(
                    "Bidirected Network",
                    payload,
                    f"bidirected_network_{keyword}_{method}",
                    "Explain the strongest keyword relationships and what support, confidence, lift, and conviction suggest.",
                )
